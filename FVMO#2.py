...
T = lambda x,y: (0.09*y+0.1)*math.sqrt(x) + (0.9-0.09*y)*x**2
...
for i in range(cell_centers.shape[0]):
    for j in range(cell_centers.shape[1]):
        ...
        if (i==0) or (i==ny-2) or (j==0) or (j==nx-2):
            vector[meshToVec(i,j)]= h_y*0.5*(base+top)*u_lam(cell_centers[i,j,0],cell_centers[i,j,1])
...
f_center = np.zeros((cell_centers.shape[0],cell_centers.shape[1]))
u_exact_center = np.zeros((cell_centers.shape[0],cell_centers.shape[1]))

for i in range(num_unknowns):
    u_center[vecToMesh(i)] = u[i]
    f_center[vecToMesh(i)] = vector[i]
    ii,jj = vecToMesh(i)
    pos = cell_centers[ii,jj]
    u_exact_center[vecToMesh(i)] = u_lam(pos[0],pos[1])
...
ax.plot_surface(cell_centers[:,:,0],cell_centers[:,:,1],u_exact_center,cmap='viridis', edgecolor='none')
